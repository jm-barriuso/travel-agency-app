...
module.exports.getAverageRatingReview = async (req,res) =>{
    try{
        const { id } = req.params;
        const averageRating = await Travel.findById(id, [
            { $unwind: "$comments" },
            { $group : { _id: "$packageName", avgRating : {  $avg : "$comments.rating" } } }
        ]);
        res.json({message:"este es el rating promedio",
                    avg:averageRating});
    } catch(err){
        res.status(404).json({ 
            message: 'Ups no hemos podido conseguir el promedio del raiting',
            err
        });
    }
